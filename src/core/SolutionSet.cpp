...
Solution * SolutionSet::get(int i) {
  ...
  //cout << "solutionsList_.size() " << solutionsList_.size() << " i " << i << endl;
    
  if (i < 0 || i >= solutionsList_.size()) {
    cout << "Error in class SolutionSet trying to access to an element out of range " << i << endl;
    exit(-1);
  }
...
/*Imprime en forma Ordenada Numericamente los Objetivos */

void SolutionSet::printObjectivesToFile2(string file){
  vector <double> Obj1;
  vector <double> Obj2;
  
  for (int i = 0; i < solutionsList_.size(); i++) {
      Obj1.push_back(solutionsList_[i]->getObjective(0));
      Obj2.push_back(solutionsList_[i]->getObjective(1)*-1);
  }
