...
#ifndef PAIRINITIALIZER_H
#define PAIRINITIALIZER_H
...
class PairInitializer {
    public:
        ...
        IntVecRef             Get_BranchIdxs()                         { return BranchIdxs;            }
        ...
        void InitBranch(int branch);                         //Function to Initialize the Branch
        void InitBranchIdxs();                               //Function to Initialize the BranchIdxs
        ...
        void MakeProperInitializations(int branch, IntMatRef Homologies); //Function to Call the above Initialization Functions in the Proper Order so as to ensure that all inter-dependencies between successive calls are met. The argument defines which branch will be initialized
        ...
        void PrintBranch();                      //Function to Print which Branch is currently being examined
        void PrintBranchIdxs();                  //Function to Print the BranchIdxs of the current realignment region
    ...
    private:
        ...
        IntVec                BranchIdxs;              //The Branch indices depending on which branch of the tree is being re-aligned
        int                   Branch;                  //An integer value representing the branch to be re-aligned. 1 (E-A), 2 (E-B), 3 (E-F), 4 (F-C), 5 (F-D), 6 (F-E)
