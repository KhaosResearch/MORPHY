...
class PairAligner {
    public:
        ...
        PairAligner();                              //Default Constructor
        virtual ~PairAligner();                     //Virtual Decontructor
        PairAligner(const PairAligner& other);          //Copy Constructor to Clone objects
        bool operator==  (const PairAligner&);      //Definition of The equality   (==) operator
        bool operator!=  (const PairAligner&);      //Definition of The inequality (!=) operator
        ...
        void   MakeTransition(IntVecRef CurIdxs, LongInt CurPosIn_1D, int stx, bool HasExtraEndState); //Given Homologies: Overloaded Function to make the transition from current state to the next state (for pairwise alignment)
