...
void PairAligner::InitParameters(Parameters* val) {
    Params = val;
}

//Function to Initialize the Substitution Model
void PairAligner::InitSubModel(F84Model* val1, TN93Model* val2, GTRModel* val3) {
    f84  = val1;
    tn93 = val2;
    gtr  = val3;
}
...
void PairAligner::InitHomologyIndices(IntCMatRef Homologies) {
...
void PairAligner::InitProbabilities(string startSoanLbl, string startTihlLbl, IntVecRef startIdxs) {
    //Fix for the direct transition to the non-emitting state when end soan is He.
    if (EndPosIn_ND[EndPosIn_ND.size()-1]==2) {
        EndPosIn_ND[0] -= 1;
        Dimensions[0] -= 1;
    }
    //End of fix
...
void PairAligner::InitSubMatrices(DoubleCMatRef val) {
    SubMats = val;
}
...
void PairAligner::GetAllReachableSoans(vector<string>& soans, const vector<string>& tihls) {
    stringstream ss;
    ss.str("");
    for (unsigned int i=0; i<Dimensions.size(); i++) {
        ss<<"H";
...
void PairAligner::GetAllPossibleTriplets(const vector<string>& soans, const vector<string>& tihls) {
    idx_R = -1;
    idx_C = -1;
    int pos;
    for (unsigned int i=0; i<soans.size(); i++) {
        ...
        if (idx_R !=-1) continue;
        if (StringCount(soans[i], "e")==soans[i].size()-1) {
            idx_R  = i;
            idx_C  = pos;
        }
    }
...
bool PairAligner::CanTihlBeAppliedToSoan(string soan, string tihl) {
...
string PairAligner::ApplyTihlToSoan(string soan, string tihl) {
    stringstream ss;
    ss.str("");
...
string PairAligner::ApplyTihlToSoan(string soan, string tihl, bool PairWise) {
    stringstream ss;
    ss.str("");
    for (unsigned int i=0; i<tihl.size(); i++) {
        switch (soan.at(i)) {
...
Triplet* PairAligner::Get_ToNonEmmitingTriplet(int r_idx) const {
    for (int i=0; i<Triplets[r_idx].size(); i++) {
        if ( StringCount(Triplets[r_idx][i]->Get_SoanAfter(), "e")==Triplets[r_idx][i]->Get_SoanAfter().size()-1 ) return Triplets[r_idx][i];
    }
    return NULL;
}
...
Triplet* PairAligner::Get_FromNonEmmitingTriplet(string sAfter, IntCVecRef idxs) const {
...
//ebB is at index 40....)
int PairAligner::IndexOfSoan(string soan) {
...
bool PairAligner::IsValidTransition(IntVecRef NextIdxs, string tihl) {
...
double PairAligner::GetSubstitutionProbability(string tihlLbl, const vector<int>& Idxs, int stx) const {
    double subProb = 1;
    ...
    if (tihlLbl.at(1)== 'H') {
        ...
        for (int j=0; j<4; j++) { //For every nucleotide A,C,G,T
            tempSum += FPTables[0][j][Idxs[0]] * FPTables[1][j][Idxs[1]];
        }
        subProb *= tempSum;
    }else if (tihlLbl.at(1)== 'B' || tihlLbl.at(1)== 'N') {
        double tempSum = 0;

        for (int j=0; j<4; j++) { //For every nucleotide A,C,G,T
            tempSum += Params->Get_BaseFreq()[j] * FPTables[1][j][Idxs[1]];
        }
        subProb *= tempSum;
    }
    return subProb;
...
Triplet* PairAligner::GetEndStateTriplet(string StartSoanLbl) {
    for (int i=0; i<Triplets.size(); i++) {
...
void PairAligner::EstimateProbabilities(int choice, bool HasExtraEndState) {
    if (SaveToLogFile) {
    ...
    for( map<LongInt, ProbabilityObject* >::iterator ii=Probabilities.begin(); ii!=Probabilities.end(); ++ii) {
        ...
        switch (choice) {
        case 0: //Probabilities Ignoring Given Homologies - Forward Algorithm (For pairwise alignment)
            MakeTransition(CurIdxs, CurPosIn_1D, 0, HasExtraEndState);
            break;

        case 1: //Probabilities Considering Given Homologies - Forward Algorithm (For pairwise alignment)
...
//Pairwise Alignment: Function to make the transition from current state to the next state
void PairAligner::MakeTransition(IntVecRef CurIdxs, LongInt CurPosIn_1D, int stx, bool HasExtraEndState) {
    /** Log Visiting States */
    ...
    for (unsigned int i=0; i<Triplets[CurIdxs[CurIdxs.size()-1]].size(); i++) {
        ...
        if (stx==1) {
            //if (CurPosIn_1D==183) { cout<<"NextIdxs:\n"<<NextIdxs<<endl<<"Is Valid? -> "<<IsValidTransition(NextIdxs, currentTriplet->Get_Tihl())<<endl;  }
...
void PairAligner::TransitToNonEmittingState() {
    ProbabilityObject* prob = new ProbabilityObject();
    prob->Set_OverallProbability(log(0.0));
...
void PairAligner::GetEvolHistories(StringMatRef evolHist, DoubleVecRef probs) {
...
/*
void PairAligner::GetEvolHistory(StringVecRef hist, double& prob, bool FixNs) {
    prob = log(0);
...
//Function to sample only one evolutionary history based on the calculated Probabilities
void PairAligner::GetEvolHistory(StringVecRef hist, double& prob) {
    prob = log(0);
...
void PairAligner::GetEvolHistory(StringVecRef hist, double& prob, StringVecRef soans, StringVecRef tihls, IntMatRef idxs) {
...
void PairAligner::SaveResultsToFile(string filename) {
...
void PairAligner::PrintParameters() {
    if (Params==NULL) {
...
void PairAligner::PrintSubstitutionMatrices() {
...
void PairAligner::PrintFPTables() {
    cout<<"\nFragment's FPTables:\n--------------------\n";
...
void PairAligner::PrintDimensions() {
    cout<<"\nDimensions:\n-----------\n<";
...
void PairAligner::PrintTriplets() {
    cout<<"\nTriplets:\n---------\n";
    for (int i=0; i<Triplets.size(); i++) {
        double sum = 0;
        for (int j=0; j<Triplets[i].size(); j++) {
...
void PairAligner::PrintModels() {
    cout<<"\n"<< (*Fid)<<endl;
}

//Function to Print the Alignment Boundaries
void PairAligner::PrintBoundaries() {
    cout<<"\nAlignment Region:\n-----------------\n";
    cout<<"Start State:  <";
...
//Function to call the above printing functions in order to output the curretn PairAligner instance setup
void PairAligner::PrintStatus() {
    PrintBoundaries();
    PrintDimensions();
    PrintLengths();
    //PrintFPTables();
...
ostream& operator<<(ostream& ostr, const PairAligner& aligner) {
